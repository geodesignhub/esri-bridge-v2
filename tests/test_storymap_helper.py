...
class TestStoryMapPublisher(unittest.TestCase):
    ...
    def test_add_text(self, mock_storymap_class, mock_text_class):
        ...
        panel = {"type": "text", "style": "paragraph", "text": "Test Paragraph"}
        publisher._add_text(panel)

        mock_text_class.assert_called_once_with(text="Test Paragraph", style=TextStyles.PARAGRAPH)
    ...
    def test_add_image(self, mock_storymap_class, mock_image_class):
        ...
        with patch.object(publisher, '_process_image', return_value='/path/to/processed/image.jpeg'):
            panel = {"type": "image", "url": "https://example.com/image.jpg", "caption": "Test Image"}
            publisher._add_image(panel)

            mock_image_class.assert_called_once_with(path='/path/to/processed/image.jpeg')
            mock_storymap_instance.add.assert_called_once_with(mock_image_class.return_value)
    ...
    def test_add_map(self, mock_storymap_class, mock_item_class, mock_map_class):
        ...
        panel = {"type": "map", "item_id": "test_map_item_id", "caption": "Test Map"}
        publisher._add_map(panel)

        mock_item_class.assert_called_once_with(gis=self.mock_gis, itemid="test_map_item_id")
    ...
    def test_process_image(self, mock_image_open, mock_os_remove, mock_tempfile, mock_requests_get):
        ...
        # Assertions
        mock_requests_get.assert_called_once_with("https://example.com/image.jpg", stream=True, headers={
        ...
        mock_image_instance.convert.return_value.save.assert_called_once_with('/tmp/tempfile.jpeg', "JPEG")
    ...
    def test_add_separator(self, mock_storymap_class, mock_separator_class):
        ...
        mock_storymap_instance.add.assert_called_once_with(mock_separator_class.return_value)

    @patch('storymap_helper.Image')
    @patch('storymap_helper.Gallery')
    @patch('storymap_helper.StoryMap')
    def test_add_gallery(self, mock_storymap_class, mock_gallery_class, mock_image_class):
        mock_storymap_instance = mock_storymap_class.return_value
        publisher = self.create_publisher()
        publisher._storymap = mock_storymap_instance
        ...
        with patch.object(publisher, '_process_image', side_effect=['/path/to/image1.jpeg', '/path/to/image2.jpeg']):
            panel = {
                "type": "gallery",
                "images": [
            ...
            mock_storymap_instance.add.assert_called_once_with(mock_gallery_class.return_value)
            ...
            total_images_added = sum(len(call_args[0][0]) for call_args in mock_gallery_class.return_value.add_images.call_args_list)
    ...
    def test_add_table(self, mock_storymap_class, mock_table_class):
        ...
        # Verify that Table is initialized with correct rows and columns
        mock_table_class.assert_called_once_with(rows=3, columns=2)
        # Verify that table is added to the story map
        mock_storymap_instance.add.assert_called_once_with(mock_table_class.return_value)

        # Since the content setting is commented out in your code, we can't test table content directly
    ...
    def test_add_table_no_headers(self, mock_storymap_class, mock_table_class):
        ...
        with patch('storymap_helper.logger') as mock_logger:
            publisher._add_table(panel)
            mock_logger.warning.assert_called_with("Table headers or rows are missing. Skipping this panel.")
    ...
    def test_set_cover(self, mock_storymap_class):
        ...
        expected_image_url = cover.get("cover_image_url", "https://story.maps.arcgis.com/sharing/rest/content/items/2fcc801c983a402eb427dd5cd07ee759/data")
    ...
    def test_add_text_invalid_style(self, mock_storymap_class, mock_text_class):
        ...
        panel = {"type": "text", "style": "invalid_style", "text": "Test Text"}
        with patch('storymap_helper.logger') as mock_logger:
            publisher._add_text(panel)

            mock_logger.warning.assert_called_with("Invalid text style: INVALID_STYLE, defaulting to PARAGRAPH")
            mock_text_class.assert_called_once_with(text="Test Text", style=TextStyles.PARAGRAPH)
            mock_storymap_instance.add.assert_called_once_with(mock_text_class.return_value)
    ...
    def test_add_map_no_item_id(self, mock_storymap_class):
        ...
        panel = {"type": "map", "caption": "Test Map"}
        with patch('storymap_helper.logger') as mock_logger:
            publisher._add_map(panel)
    ...
    def test_add_image_no_url(self, mock_storymap_class, mock_image_class):
        ...
        panel = {"type": "image", "caption": "Test Image"}
        with patch('storymap_helper.logger') as mock_logger:
            publisher._add_image(panel)
    ...
    @patch('storymap_helper.os.remove')
    @patch('storymap_helper.PILImage.open')
    def test_process_image_download_fail(self, mock_image_open, mock_os_remove, mock_tempfile, mock_requests_get, mock_storymap_class):
        ...
        with patch('storymap_helper.logger') as mock_logger:
            ...
            mock_logger.error.assert_called_with("Failed to process image: https://example.com/image.jpg - Download failed")
    ...
    @patch('storymap_helper.os.remove')
    @patch('storymap_helper.PILImage.open')
    def test_process_image_conversion_fail(self, mock_image_open, mock_os_remove, mock_tempfile, mock_requests_get, mock_storymap_class):
        ...
        with patch('storymap_helper.logger') as mock_logger:
            result = publisher._process_image("https://example.com/image.jpg")
            self.assertIsNone(result)
            mock_logger.error.assert_called_with("Failed to process image: https://example.com/image.jpg - Conversion failed")
    ...
    def test_add_image_process_image_fail(self, mock_storymap_class):
        ...
        with patch.object(publisher, '_process_image', return_value=None):
            ...
            with patch('storymap_helper.logger') as mock_logger:
                publisher._add_image(panel)
                mock_logger.error.assert_called_with("Failed to add image: https://example.com/image.jpg - Failed to process image: https://example.com/image.jpg")
    ...
    @patch('storymap_helper.Map')
    @patch('storymap_helper.StoryMap')
    def test_add_map_exception(self, mock_storymap_class, mock_map_class, mock_item_class):
        mock_storymap_instance = mock_storymap_class.return_value
        publisher = self.create_publisher()
        publisher._storymap = mock_storymap_instance
        ...
        with patch('storymap_helper.logger') as mock_logger:
            publisher._add_map(panel)
            mock_logger.error.assert_called_with("Failed to add map: Map creation failed")
            # Ensure no map is added
            mock_storymap_instance.add.assert_not_called()

    @patch('storymap_helper.Table')
    @patch('storymap_helper.StoryMap')
    def test_add_table_invalid_rows_columns(self, mock_storymap_class, mock_table_class):
        mock_storymap_instance = mock_storymap_class.return_value
        publisher = self.create_publisher()
        publisher._storymap = mock_storymap_instance
        ...
        with patch('storymap_helper.logger') as mock_logger:
            publisher._add_table(panel)
            mock_logger.warning.assert_called_with("Invalid number of rows (10). Must be between 1 and 9. Skipping.")
            mock_table_class.assert_not_called()
            mock_storymap_instance.add.assert_not_called()
        ...
        with patch('storymap_helper.logger') as mock_logger:
            publisher._add_table(panel)
            mock_logger.warning.assert_called_with("Invalid number of columns (9). Must be between 1 and 8. Skipping.")
            mock_table_class.assert_not_called()
            mock_storymap_instance.add.assert_not_called()
